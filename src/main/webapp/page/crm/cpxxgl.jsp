...
<body>
    ...
    <div class="tools">
        ...
        <li  onclick="location.href='${pageContext.servletContext.contextPath}/cpxxgl.do?method=goto_add'"><span><img src="${pageContext.servletContext.contextPath}/images/t01.png" /></span>新增产品</li>
        </c:if>
    ...
    <li><label>产品编码</label><input name="code" type="text" class="scinput" /></li>
    <li><label>规格型号</label><input name="model" type="text" class="scinput" /></li>
    <%-- <li><label>客户来源</label>  
    <div class="vocation">
	<select name="">
    ...
    <table class="tablelist" id="tablelist">
        ...
        <c:forEach items="${page.items}" var="o" varStatus="status">
          ...
          <td style="width:150px;">${o.model}</td>
          <td style="width:200px;">${o.unit.name}</td>
          <td>${o.kcsl}</td>
          <td style="width:180px;">${o.kykc}</td>
          <td>${o.price}</td>
          <td>${o.creator.nickName}</td>
          <td>${o.createTime}</td>
          <td>
             <a href="${pageContext.servletContext.contextPath}/cpxxgl.do?method=detail&id=${o.id}">查看</a>&nbsp;
             <c:if test="${sessionScope.userRole.role.name eq '管理员'}">
   ...
   <%-- <c:if test="${page.pageCount gt 0}">
    <div class="pagin">
        ...
        <c:if test="${page.pageCount>7}">
            <c:if test="${page.current le page.pageCount-3}">
              <li class="paginItem"><a href="javascript:;"><span class="pagepre"></span></a></li>
             ...
             <c:if test="${page.current gt page.pageCount-3}">
                 <li class="paginItem"><a href="javascript:;"><span class="pagepre"></span></a></li>
              <c:forEach begin="${page.pageCount-6}" step="1" var="index" end="${page.pageCount}" >
	             <c:if test="${page.current eq index}">
	              <li class="paginItem current"><a href="javascript:;">${index}</a></li>
	             </c:if>
	             <c:if test="${page.current ne index}">
 ...
 <p:page pages="${page}" link="cpxxgl.do?method=listGoods" prev="上一页" first="首页" last="尾页" next="下一页" ulClass="paginList" />
